...
# n: Integer to retrieve the digits for
def toList(n):
	digits = []
	while n > 0:
		digits.append(n % 10)
...
def isPalindrome(n):
	dn = toList(n)
	for i in range(len(dn) // 2):
		if dn[i] != dn[len(dn) - 1 - i]:
			return False
	return True

# A number is assumed to be Lychrel unless proven otherwise.
# For this problem, we assume that it is Lychrel if it does not
# yield a palindrome in less than fifty iterations.
def isLychrel(n):
	for i in range(50):
		# Reverse n
		digits = toList(n)
		...
		# Add to n, to form new iteration
		n = n + n_rev
		if isPalindrome(n):
			return False
	return True
